...
EXTRA_FLAGS="--ssl-verify-server-cert=off"
BACKUP_STATUS="true"
BACKUP_FAIL_REASON=""
...
mongodump --host $DB_SERVER --port $DB_PORT --username $DB_USERNAME --password $DB_PASSWORD --archive > $TEMP_BACKUP_DIR/$DATE/dump.dump
if [ $? -ne 0 ]; then
